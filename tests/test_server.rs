...
#[test]
fn threadpool_test_server() -> izanami::Result<()> {
    let mut server = izanami::test::server(Echo)?;
...
#[test]
fn singlethread_test_server() -> izanami::Result<()> {
    let mut server = izanami::test::local_server(Echo)?;
