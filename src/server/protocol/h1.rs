...
use {
    crate::server::{
        upgrade::HttpUpgrade,
        util::{HttpService, RewindIo},
        BoxedStdError, Connection,
    },
    crate::{body::HttpBody, util::MapAsyncOptExt},
    bytes::{Buf, Bytes},
...
impl H1 {
    ...
    where
        ...
        <S::ResponseBody as HttpUpgrade<RewindIo<I>>>::UpgradeError: Into<BoxedStdError>,
    ...
    fn poll_complete_inner(&mut self) -> Poll<(), BoxedStdError> {
        loop {
            ...
            match self.state {
                ...
                State::Upgraded(ref mut conn) => {
                    return conn.poll_close().map_err(Into::into);
                }
            ...
            self.state = match std::mem::replace(&mut self.state, State::Closed) {
                ...
                State::WillUpgrade { rewind, .. } => {
                    ...
                    match body.upgrade(rewind) {
                        Ok(conn) => State::Upgraded(conn),
                        Err(io) => State::Shutdown(io),
                    }
                }
    ...
    Bd::UpgradeError: Into<BoxedStdError>,
{
    ...
    fn graceful_shutdown(&mut self) {
        match self.state {
            ...
            State::Upgraded(ref mut conn) => conn.graceful_shutdown(),
            _ => (),
        }
    }
}
